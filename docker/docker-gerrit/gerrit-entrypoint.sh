...
if [ "$1" = '/var/gerrit/gerrit-start.sh' ]; then
  if [ -z "$(ls -A "$GERRIT_SITE")" ]; then
    ...
    java -jar "${GERRIT_WAR}" init --batch --no-auto-start --install-plugin replication --install-plugin commit-message-length-validator  -d "${GERRIT_SITE}"
  ...
  if [ "${DATABASE_TYPE}" = 'postgresql' ]; then
    ...
    [ -z "${DB_ENV_DB_NAME}" ] || git config -f "${GERRIT_SITE}/etc/gerrit.config" database.database "${DB_ENV_DB_NAME}"
    [ -z "${DB_ENV_DB_USER}" ] || git config -f "${GERRIT_SITE}/etc/gerrit.config" database.username "${DB_ENV_DB_USER}"
    [ -z "${DB_ENV_DB_PASS}" ] || git config -f "${GERRIT_SITE}/etc/secure.config" database.password "${DB_ENV_DB_PASS}"
  fi
